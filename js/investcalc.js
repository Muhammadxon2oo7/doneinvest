...
function updateSavingsLabel() {
    $('#savingsLabel').text('Har bir ' + freqtext[saveFreq]+'dagi jamg`arma');
}
...
function updateCalc() {
    ...
    if (altopt != 1) timeunits = timeunits + "";

    var output;
    if (altopt == 0) {
        output = "Sarmoya kiritmaslik yaxshiroqdir, chunki kutilgan daromad jamg'armalar bo'yicha foiz stavkasidan oshmaydi.";
        if (growth > interest) {
            output += " (E'tibor bering, oylik kompensatsiyadan so'ng bu erda foiz stavkasi keladi "+(100*(Math.pow(1+interest/12,12)-1)).toFixed(2)+"% p.a.)";
        }
    } else {
        output = "Har "+(altopt == 1 ? '' : altopt+" ")+timeunits +" da bir marta "+parcelsize+'$ investitsiya qiling';
        var extra = '';
        var hasinterest = (parcelsize > opt*savings);
        if (brokerage > 0 && hasinterest) {
            extra = ' Ushbu summa sarflangan komissiya va $'+Math.round(parcelsize-opt*savings)+' foiz ko`rinishidagi foydani o`z ichiga oladi..';
        } else if (brokerage > 0) {
            extra = 'Ushbu miqdor komissiyani o`z ichiga oladi ';
        }
...
function updateChart(opt, interval, savings, interest, growth, brokerage) {
    ...
    for (var j=0; j<nvals.length; j++) {
        ...
        if (altopt != 1) timeunits = timeunits + "";
        var label = 'Har '+(altopt == 1 ? '' : altopt+" ")+timeunits;
        labels.push(label);
    ...
    if (typeof chart == 'undefined') {
        ...
        chart = new Chart(ctx, {
            ...
            options: {
                title: {
                    display: true,
                    text: 'O`rtacha 10 yilda umumiy jamg`arma ($)'
                },
                legend: {
                    display: false
                },
    ...
    chart.data.datasets[0].label = '10 yildan keyin';
